...
class HomeFeed < Feed
  ...
  def get_recommendations(status_ids, limit, max_id = nil, since_id = nil, min_id = nil)
    ...
    response = Net::HTTP.post(URI(ENV["RECOMMENDER_URL"] + "accounts/#{@account.id}/create-sorted-timeline"), { "status_ids": status_ids, "ranking_score_treshold": 0 }.to_json, headers)
